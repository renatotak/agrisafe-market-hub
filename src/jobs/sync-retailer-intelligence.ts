...
export async function runSyncRetailerIntelligence(supabase: SupabaseClient): Promise<JobResult> {
  ...
  try {
    ...
    const uids = batch.map((r) => r.entity_uid)
    const { data: existing } = await supabase
      .from('retailer_intelligence')
      .select('entity_uid, analyzed_at')
      .in('entity_uid', uids)
    ...
    const toAnalyze = batch.filter((r) => !recentlyAnalyzed.has(r.entity_uid)).slice(0, BATCH_SIZE)
    ...
    for (const retailer of toAnalyze) {
      try {
        ...
        const { count: branchCount } = await supabase
          ...
          .eq('entity_uid', retailer.entity_uid)
        ...
        const { data: indRels } = await supabase
          ...
          .select('industry_id')
          .eq('retailer_entity_uid', retailer.entity_uid)
        ...
        const { error: upsertError } = await supabase.from('retailer_intelligence').upsert({
          entity_uid: retailer.entity_uid,
        ...
        }, { onConflict: 'entity_uid' })

        if (upsertError) errors.push(`${retailer.entity_uid}: ${upsertError.message}`)
        else analyzed++
      } catch (e: any) {
        errors.push(`${retailer.entity_uid}: ${e.message}`)
      }
    }
